...
export async function initializeGeneralSettings() {
  try {
    ...
    if (existingSettings) {
      console.log(
        colors.bgWhite.cyan.bold('La configuración general ya existe.')
      )
      ...
      console.log(
        colors.green.bold('Configuración general inicializada exitosamente.')
      )
      return newSettings.id
    }
  } catch (error) {
    console.error(
      colors.bgYellow.red('Error al inicializar la configuración general:'),
      error
    )
    throw error
  }
...
export async function initializeSecuritySettings() {
  try {
    ...
    if (existingSecuritySettings) {
      console.log(
        colors.bgWhite.cyan.bold('La configuración de seguridad ya existe.')
      )
      ...
      console.log(
        colors.green.bold(
          'Configuración de seguridad inicializada exitosamente.'
        )
      )
    ...
    console.error(
      colors.bgYellow.red(
        'Error al inicializar la configuración de seguridad:'
      ),
      error
    )
    throw error
...
export async function initializeActiveModules() {
  try {
    ...
    if (existingActiveModules) {
      console.log(
        colors.bgWhite.cyan.bold('Los módulos activos ya están definidos.')
      )
      ...
      console.log(
        colors.green.bold('Módulos activos inicializados exitosamente.')
      )
      return newActiveModules.id
    }
  } catch (error) {
    console.error(
      colors.bgYellow.red('Error al inicializar los módulos activos:'),
      error
    )
    throw error
  }
...
export async function initializeApplication() {
  try {
    ...
    console.error(
      colors.bgYellow.red('Error al inicializar la aplicación:'),
      error
    )
  }
}
