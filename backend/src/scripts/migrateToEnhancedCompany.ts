...
export async function migrateToEnhancedCompany(): Promise<void> {
  try {
    ...
    // Obtener todas las empresas del modelo Company
    ...
    for (const company of companies) {
      try {
        ...
        const enhancedCompanyData: Partial<IEnhancedCompany> = {
          ...
          status: 'active',
          plan: 'professional', // Asignar plan profesional por defecto

          // Configuraciones
          settings: {
            businessType: mapIndustryToBusinessType(company.industry),
