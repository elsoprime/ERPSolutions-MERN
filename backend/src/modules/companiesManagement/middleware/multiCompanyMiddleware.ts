...
interface EnhancedCompanyContext {
  ...
  slug: string;
  name: string;
  plan: Types.ObjectId; // ✅ Cambiado de string a ObjectId
  status: string;
  canAccess: boolean;
}
...
export class MultiCompanyMiddleware {
  ...
  static requireCompanyPermission(permission: CompanyPermission) {
    return (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        console.error(
          "Error en verificación de permisos empresariales:",
          error
        );
        res.status(500).json({
          error: "Error interno del servidor",
  ...
  static requireCompanyAdmin() {
    return MultiCompanyMiddleware.requireCompanyPermission(
      "users.assign_roles"
    );
  }
