...
// ====== ACTIVE COMPANIES ONLY HOOK (Para formularios) ======
export function useActiveCompanies() {
  const query = useQuery({
    queryKey: [...COMPANY_QUERY_KEYS.all, "active-only"],
    queryFn: () =>
      MultiCompanyAPI.getAllCompanies({ status: CompanyStatus.ACTIVE }),
    enabled: true,
    staleTime: 5 * 60 * 1000, // 5 minutos
  });
...
// ====== COMPANY CHART DATA HOOK (NUEVO) ======
/**
 * Hook especializado para obtener datos de gráficas de una compañía
 * Transforma automáticamente los datos del backend a formatos de gráficas
 */
export function useCompanyChartData(companyId: string) {
  return useQuery({
    queryKey: ["company-charts", companyId],
    queryFn: async () => {
      const data = await import("@/api/EnhancedCompanyAPI").then((mod) =>
        mod.default.getCompanyStatistics(companyId)
      );
      // Transformar automáticamente los datos
      const { transformCompanyChartStats } = await import(
        "@/utils/chartTransformers"
      );
      return transformCompanyChartStats(data);
    },
    enabled: !!companyId,
    staleTime: 5 * 60 * 1000, // 5 minutos
  });
...
// ====== COMPANIES SUMMARY WITH TRENDS HOOK (NUEVO) ======
/**
 * Hook para obtener resumen global con tendencias mensuales
 * Transforma automáticamente los datos para gráficas
 */
export function useCompaniesSummaryExtended() {
  return useQuery({
    queryKey: ["companies-summary-extended"],
    queryFn: async () => {
      const data = await import("@/api/EnhancedCompanyAPI").then((mod) =>
        mod.default.getCompaniesSummaryExtended()
      );
      // Transformar automáticamente los datos
      const { transformCompaniesSummary } = await import(
        "@/utils/chartTransformers"
      );
      return transformCompaniesSummary(data);
    },
...
export function useCompanyMutations() {
  ...
  const updateCurrentCompanySettings = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        data.message || "Configuraciones actualizadas exitosamente"
      );
    },
    onError: (error: any) => {
      toast.error(
...
export function useCompanyForm(initialCompany?: IEnhancedCompany) {
  const [formData, setFormData] = useState<Partial<ICreateCompanyRequest>>({
    ...
    plan: initialCompany?.plan || PlanType.FREE,
    settings: {
