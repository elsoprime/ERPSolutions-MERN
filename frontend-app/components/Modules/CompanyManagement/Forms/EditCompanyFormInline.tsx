/**
 * Edit Company Form - Inline Version
 * @description: Formulario inline para editar empresas sin modal con transiciones
...
import { useEditCompanyForm } from '@/hooks/CompanyManagement/useEditCompanyForm'
...
/**
 * Componente de formulario inline de edición (sin modal) con transiciones
 */
    ...
    const renderValidationIndicator = () => {
        ...
        return (
            <div className='bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6'>
                <div className='flex items-center'>
                    <ExclamationTriangleSolidIcon className='w-5 h-5 text-amber-600 mr-2' />
        ...
        <div className='bg-white shadow-sm rounded-lg animate-fade-in'>
                ...
                <div className='flex items-center justify-between'>
                    <div>
                        ...
                        <p className='text-xs text-gray-600 mt-1'>
                            Modificando: <span className='font-medium text-blue-600'>{company.name}</span>
                        </p>
                    </div>
                    <button
                        onClick={onCancel}
            ...
            <form
                ...
                onKeyDown={(e) => {
                    ...
                    if (e.key === 'Enter') {
                        // Permitir Enter solo en textareas y en el paso final
            ...
            >
                            ...
                            <div className='mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md animate-fade-in'>
                                <div className='flex items-start gap-2'>
                                    ...
                                    <div className='text-sm text-yellow-800'>
                                        <p className='font-medium'>⚠️ Cambio de Plan Detectado</p>
                                        <p className='text-xs mt-1'>
                                            Está cambiando el plan de suscripción de esta empresa.
                                            Esto puede afectar las características y límites disponibles.
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                {formState.currentStep === 4 && (
                    <div className='space-y-6'>
                        {/* Indicador de validación mejorado */}
                        {renderValidationIndicator()}                        <h3 className='text-lg font-medium text-gray-900 border-b border-gray-200 pb-2'>
                                    ...
                                    <div>
                                        <strong>Plan:</strong>{' '}
                                        {
                                            formState.availablePlans.find(
                                                p => p._id === formState.selectedPlanId
                ...
                <div className='flex items-center justify-between mt-8 pt-6 border-t border-gray-200'>
                    ...
                    {/* Indicador de paso y cambios */}
                    <div className='flex items-center gap-2'>
                        <span className='text-xs text-gray-600'>
                            Paso {formState.currentStep} de {FORM_STEPS.length}
                        </span>
                        {(formState.isDirty || formState.planChanged) && (
                            <span className='text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full'>
                                {formState.planChanged ? 'Plan cambiado' : 'Cambios sin guardar'}
                            </span>
                        )}
                    ...
                    {formState.currentStep === FORM_STEPS.length ? (
                        <button
                            ...
                            title={!formState.isCurrentStepValid ? 'Complete todos los campos requeridos' : ''}
                        >
                    ...
                    <div className='flex items-start gap-2'>
                        ...
                        <div className='text-xs text-blue-800'>
                            <p className='font-medium'>
                                {formState.currentStep === 1 && 'Actualice la información básica de la empresa'}
                                {formState.currentStep === 2 && 'Modifique la configuración del negocio'}
                                {formState.currentStep === 3 && 'Cambie el plan de suscripción si es necesario'}
                                {formState.currentStep === 4 && 'Personalice los colores de marca'}
                            </p>
                            <p className='text-xs mt-1 text-blue-600'>
                                Solo los campos modificados serán actualizados
                            </p>
                        </div>
                    </div>
                </div>
